...
static bool checkhtkeys(HashTable* ht, pmutex_t* lock) {
    Array* arr = nilptr;
    pmutex_lock(lock);
    int rv = hashtable_get_keys(ht, &arr);
    int htsz = hashtable_size(ht);
    pmutex_unlock(lock);
    if (htsz > 0) {
        assert(arr != nilptr);
