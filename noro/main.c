...
machine* noro_machine_get(int id) {
    machine* mc = 0;
    (machine*)hashtable_get(nr->mcs, (void*)id, &mc);
    return mc;
}


void noro_post(coro_func fn, void*arg) {
    int id = noro_nxtid(nr);
...
void* noro_processor0(void*arg) {
    ...
    linfo("%d\n", mc->id);
    pthread_cond_signal(&nr->noroinitcd);
...
HashTableConf* noro_dft_htconf() { return &nr->htconf; }
