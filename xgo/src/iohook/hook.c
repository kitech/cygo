...
static iohook_allocator iohk_allocer = {.mallocfn = malloc,
...
static iohook_yielder iohk_yielder = {};
static int (*crn_in_procer)() = nilptr;
static int (*crn_procer_yield)(long, int) = nilptr;
static int (*crn_procer_yield_multi)(int, int, long*, int*) = nilptr;
/* static void (*hookcb_oncreate)(int, int, int, int, int, int) = nilptr; */
/* static void (*hookcb_onclose)(int) = nilptr; */
/* static void (*hookcb_ondup)(int, int) = nilptr; */
...
static iohook_fdinfo iohk_fdinfos[5120] = {-1};

static iohook_fdinfo* hookcb_get_fdcontext(int fd) {
    iohook_fdinfo* ctx = &iohk_fdinfos[fd];
    if (ctx->fd < 0) {
        linfo("wtt %d=?%d\n", fd, ctx->fd);
        assert(ctx->fd >= 0);
    }
...
static int hookcb_fd_set_nonblocking(int fd, int isNonBlocking) {
...
static int fdcontext_set_nonblocking(iohook_fdinfo*fdctx, bool isNonBlocking) {
    if (fdctx == 0) {
        return 0;
    }
    assert(fdctx->fd >= 0);
    return hookcb_fd_set_nonblocking(fdctx->fd, isNonBlocking);
}
...
static int fdcontext_check_file_stat(int fd) {
    iohook_fdinfo* fdctx = &iohk_fdinfos[fd];
    struct stat* oldsti = &fdctx->sti;
    struct stat newsti = {0};
    int rv = fstat(fd, &newsti);
    if (rv == 0) {
        if (newsti.st_ino != oldsti->st_ino) {
            linfo("fdstat refresh %d ino %d->%d\n", fd, oldsti->st_ino, newsti.st_ino);
            memcpy(oldsti, &newsti, sizeof(struct stat));
        }
    }else{
        linfo("fdstat error %d %s\n", fd, strerror(errno));
    }
    return 0;
}

static bool fdcontext_is_socket(iohook_fdinfo*fdctx) {
    if (fdctx == 0) return false;
    return fdctx->fdty == FDISSOCKET;
}
static bool fdcontext_is_tcpsocket(iohook_fdinfo*fdctx) {
    if (fdctx == 0) return false;
    struct stat* sti = &fdctx->sti;
    return (fdctx->fdty == FDISSOCKET || S_ISSOCK(sti->st_mode))
            && fdctx->sockty == SOCK_STREAM;
}
static bool fdcontext_is_file(iohook_fdinfo* fdctx) {
    if (fdctx == 0) return false;
    fdcontext_check_file_stat(fdctx->fd);
    struct stat* sti = &fdctx->sti;
    linfo("fdty %d %d %d, close%d ino %d\n",
          fdctx->fd, fdctx->fdty, S_ISREG(sti->st_mode), fdctx->closed, sti->st_ino);
    if (fdctx->closed) {
        // abort();
    }
    return fdctx->fdty == FDISFILE || S_ISREG(sti->st_mode);
}
static bool fdcontext_is_nonblocking(iohook_fdinfo* fdctx){
    return fdctx->isNonBlocking;
}
static int fdcontext_get_fdtype(iohook_fdinfo* fdctx){
    return fdctx->fdty;
}
...
void hookcb_oncreate(int fd, int fdty, bool isNonBlocking, int domain, int sockty, int protocol,
    ...
    fdctx->tm = time(0);
    fdctx->closed = false;
    ino_t oldino = sti->st_ino;
    fstat(fd, &fdctx->sti);
    ino_t newino = sti->st_ino;

    assert(crn_in_procer != nilptr);
    int incoro = crn_in_procer();
    linfo("fdctx new %d %p inco %d, ino %d->%d, %s:%d\n",
          fd, fdctx, incoro, oldino, newino, fun, line);
    if (incoro && (fdty == FDISSOCKET || fdty == FDXLIB))
    if (!fd_is_nonblocking(fd)) {
...
static void hookcb_onclose(int fd, const char* fun, int line) {
    iohook_fdinfo* fdctx = hookcb_get_fdcontext(fd);
    // memset(fdctx, -1, sizeof(iohook_fdinfo));
    fdctx->closed = true;
    struct stat* sti = &fdctx->sti;
    linfo("fdctx close %d, ino %d, %s:%d\n", fd, sti->st_ino, fun, line);
    // if (sti->st_ino == 3281195) {abort();}
}

static void hookcb_ondup(int from, int to) {
    iohook_fdinfo* fdctx = hookcb_get_fdcontext(from);
    iohook_fdinfo* fdctx2 = &iohk_fdinfos[to];
    memcpy(fdctx2, fdctx, sizeof(iohook_fdinfo));
    fdctx2->fd = to;
    fdctx2->tm = time(0);
}

//
static void hookcb_setin_poll(int fd, bool set, bool isread) {
    iohook_fdinfo* fdctx = hookcb_get_fdcontext(fd);
    if (isread) {
...
static bool hookcb_getin_poll(int fd, bool isread) {
    iohook_fdinfo* fdctx = hookcb_get_fdcontext(fd);
    if (isread) {
...
{
    assert(socket_f);
    // if (!socket_f) iohook_initHook();
    linfo("%d\n", pipefd[0]);

    int rv = pipe_f(pipefd);
    if (rv == 0) {
        if (crn_in_procer()) {
            hookcb_oncreate(pipefd[0], FDISPIPE, false, 0,0,0, __FUNCTION__, __LINE__);
            hookcb_oncreate(pipefd[1], FDISPIPE, false, 0,0,0, __FUNCTION__, __LINE__);
        }
    }
    return rv;
}
...
int pipe2(int pipefd[2], int flags)
{
    ...
    if (rv == 0) {
        hookcb_oncreate(pipefd[0], FDISPIPE, !!(flags&O_NONBLOCK), 0,0,0, __FUNCTION__, __LINE__);
        hookcb_oncreate(pipefd[1], FDISPIPE, !!(flags&O_NONBLOCK), 0,0,0, __FUNCTION__, __LINE__);
    }
    return rv;
}
...
int inotify_init() {
    ...
    if (rv > 0) {
        hookcb_oncreate(rv, FDISPIPE, 0, 0,0,0, __FUNCTION__, __LINE__);
    }
    return rv;
}
int inotify_init1(int flags) {
    ...
    if (rv > 0) {
        hookcb_oncreate(rv, FDISPIPE, !!(flags&IN_NONBLOCK), 0,0,0, __FUNCTION__, __LINE__);
    }
    return rv;
}
#endif
#if defined(IOHOOK_XLIB)
XOpenDisplay_t XOpenDisplay_f = NULL;
char iohook_inxlib = 0;
Display *XOpenDisplay(_Xconst char *display_name) {
    if (!XOpenDisplay_f) initHook();
    Display* dsp = NULL;
    iohook_inxlib = 1;
    dsp = XOpenDisplay_f(display_name);
    iohook_inxlib = 0;
    return dsp;
}
#endif
...
int socket(int domain, int type, int protocol)
{
    ...
    // not need check in procer, just record it
    ...
    if (sock >= 0) {
        int fdty = FDISSOCKET;
#if defined(IOHOOK_XLIB)
        if (iohook_inxlib) fdty = FDXLIB;
#endif
        hookcb_oncreate(sock, fdty, false, domain, type, protocol, __FUNCTION__, __LINE__);
...
int socketpair(int domain, int type, int protocol, int sv[2])
{
    ...
    if (rv == 0) {
        hookcb_oncreate(sv[0], FDISSOCKET, false, domain, type, protocol, __FUNCTION__, __LINE__);
        hookcb_oncreate(sv[1], FDISSOCKET, false, domain, type, protocol, __FUNCTION__, __LINE__);
    }
    return rv;
}
...
int connect(int fd, const struct sockaddr *addr, socklen_t addrlen)
{
    ...
    for (int i = 0;; i++) {
        ...
        const char* ipstr = inet_ntop(AF_INET, &sa->sin_addr.s_addr, buf, 200);
        unsigned short dstport = ntohs(sa->sin_port);
        //linfo("what addr %s %d\n", ipstr, dstport);
        ...
        if (eno != EINPROGRESS && eno != EALREADY) {
            linfo("Unknown %d %d %d %d %s %s:%d\n", fd, rv, eno, i, strerror(eno), ipstr, dstport);
            return rv;
        }
        //linfo("yield %d %d %d\n", fd, rv, eno);
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen)
{
    ...
    while(1){
        ...
        int eno = rv < 0 ? errno : 0;
        if (rv >= 0) {
            hookcb_oncreate(rv, FDISSOCKET, false, AF_INET, SOCK_STREAM, 0, __FUNCTION__, __LINE__);
...
ssize_t read(int fd, void *buf, size_t count)
{
    ...
    iohook_fdinfo* ctx = hookcb_get_fdcontext(fd);
    if (ctx == nilptr) {
...
ssize_t recv(int sockfd, void *buf, size_t len, int flags)
{
    ...
    while (1) {
        ...
        int eno = rv < 0 ? errno : 0;
        if (rv == 0) {
            hookcb_onclose(sockfd, __FUNCTION__, __LINE__);
        }
        if (rv >= 0) {
            return rv;
        }
...
ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags)
{
    ...
    for (int i = 0; ; i ++){
        ...
        iohook_fdinfo* fdctx = hookcb_get_fdcontext(sockfd);
...
#if defined(LIBGO_SYS_Linux_WIP)
...
// avoid sleep interupt
static int iohook_ensure_sleep(unsigned int seconds) {
    unsigned int leftsec = seconds;
    time_t btime = time(0);
    for (int i = 0; ; i++) {
        int rv = sleep_f(leftsec);
        time_t etime = time(0);
...
int close(int fd)
{
    linfo("%d\n", fd);
    if (!close_f) initHook();
    if (!crn_in_procer()) return close_f(fd);

    hookcb_onclose(fd, __FUNCTION__, __LINE__);
    {
        return close_f(fd);
    }
    return 0;
...
FILE* fopen(const char *pathname, const char *mode)
{
    ...
    if (fp == 0) { return 0; }
    int fd = fileno(fp);
    hookcb_oncreate(fd, FDISFILE, 0, 0, 0, 0, __FUNCTION__, __LINE__);
...
int eventfd(unsigned int initval, int flags) {
    ...
    */
    ...
    if (rv != 0) {
        hookcb_oncreate(rv, EVENTFD, 0, 0,0,0, __FUNCTION__, __LINE__);
    }
    return rv;
}
...
#endif
#if defined(IOHOOK_XLIB)
        XOpenDisplay_f = (XOpenDisplay_t)dlsym(RTLD_NEXT, "XOpenDisplay");
#endif
...
// default for example
static int iohook_incoro_default() { return 123; }
static int iohook_yield_default(long fd, int typ) { return 0; }
static int iohook_yield_multi_default(int typ, int argc, long* fds, int*types) { return 0; }

static int iohook_isInit = 0;
void iohook_initHook(iohook_yielder* yielder, uint32_t yldsz, iohook_allocator* allocer)
{
    // linfo("ddd %p\n", yielder);
    assert(yielder != nilptr);
    assert(yldsz == sizeof(iohook_yielder));
    iohk_yielder = *yielder;
    crn_in_procer = yielder->incoro;
    crn_procer_yield = yielder->yield;
    crn_procer_yield_multi = yielder->yield_multi;

    if (allocer != nilptr) {
        iohk_allocer = *allocer;
    }

    iohook_isInit = iohook_doInitHook();
    (void)iohook_isInit;
}

void iohook_initHook2(void* incoro, void* getcoro, void* yield, void* yield_multi,
                      // void* resumeone,
                      void* mallocfn, void* callocfn, void* reallocfn, void* freefn)
{
    // linfo("ddd %p\n", incoro); // cannot write
    assert(incoro != nilptr);

    iohk_yielder.incoro = incoro;
    iohk_yielder.getcoro = getcoro;
    iohk_yielder.yield = yield;
    iohk_yielder.yield_multi = yield_multi;
    crn_in_procer = incoro;
    crn_procer_yield = yield;
    crn_procer_yield_multi = yield_multi;

    if (mallocfn != NULL) {
        iohk_allocer.mallocfn = mallocfn;
        iohk_allocer.callocfn = callocfn;
        iohk_allocer.reallocfn = reallocfn;
        iohk_allocer.freefn = freefn;
    }
